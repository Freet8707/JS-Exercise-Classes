...
class Car {
  ...
  drive(distance){
    ...
    let milesAllowed = this.tank * this.milesPerGallon;
    if(milesAllowed / distance >= 1){
      return this.tank = this.tank - distance / this.milesPerGallon;
    } else {
      this.tank = this.tank - distance / this.milesPerGallon;
      return `I ran out of fuel at ${milesAllowed} miles`;
    }
  }
}
...
class Student extends Lambdasian{
  ...
  PRAssignment(subject){return `${this.name} has submitted PR for ${subject}`;};

  sprintChallenge(subject){return `${this.name} has has begun sprint challenge on ${subject}`;};
}
